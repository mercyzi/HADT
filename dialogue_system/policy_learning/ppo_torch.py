...
class PPO(object):
    def __init__(self, input_size, hidden_size, output_size, parameter):
        ...
        self.actor = ActorSoftmax(input_size,output_size, hidden_dim = hidden_size).to(self.device)
        self.critic = Critic(input_size,1,hidden_dim=hidden_size).to(self.device)
        print(self.actor)
    ...
    def predict_eval(self,state,mask_human):
        ...
        # assert 0
        end4 = time.time()
        # print('Master程序运行时间为: %s Seconds'%(end4-end3))
        # print('Master程序运行时间为: %s Seconds'%(end3-end2))
        # print('Master程序运行时间为: %s Seconds'%(end2-end1))
        # assert 0
