...
class RunningSteward(object):
    ...
    def __init__(self, parameter, checkpoint_path):
        ...
        self.SC = 1
        self.record_early_stop = {}
        self.group_9 = {'1':[],'4':[], '5':[],'6':[], '5':[], '6':[], '7':[],'12':[], '13':[], '14':[], '19':[]}
        ...
        # assert 0
        self.cur_mr2_group = {'1':[],'2':[], '3':[],'4':[], '5':[], '6':[], '7':[],'12':[], '13':[], '14':[], '19':[],'99':[] }
        ...
        #     expl = {}
        #     impl = {}
        #     slot_s = disease_symptom[goal['disease_tag']]['symptom']
        ...
        #         if k in slot_s.keys():
        #             impl[k] = v
        #     goal['goal']['explicit_inform_slots'] = expl
        #     goal['goal']['implicit_inform_slots'] = impl 
        
        # for item in re_train_set:
        # a = [0.852, 0.957, 0.87, 0.649, 0.944, 0.308, 0.621, 0.77, 0.875, 0.696, 0.769, 0.931, 0.774, 0.891, 0.486, 0.701, 0.928, 0.625, 0.702, 0.943, 0.709, 0.509, 0.629, 0.778, 0.809, 0.597, 0.718, 0.878, 0.769, 0.554, 0.836, 0.622, 0.695, 0.652, 0.87, 0.797, 0.853, 0.787, 0.909, 0.875, 0.831, 0.746, 0.681, 0.803, 0.761, 0.438, 0.945, 0.75, 0.241, 0.486, 0.855, 0.471, 0.797, 0.647, 0.5, 0.311, 0.864, 0.932, 0.852, 1.0, 0.656, 0.831, 0.817, 0.729, 0.929, 0.725, 0.862, 0.729, 0.852, 0.973, 0.873, 1.0, 0.707, 1.0, 0.943, 0.946, 0.919, 0.952, 0.983, 1.0, 0.672, 0.466, 0.619, 0.757, 0.891, 0.885, 0.8, 0.838, 0.808, 0.571]
        # b = [1.0, 1.0, 0.986, 0.784, 0.944, 0.354, 0.667, 0.82, 0.917, 0.855, 0.923, 0.958, 0.919, 0.938, 0.514, 0.821, 0.928, 0.75, 0.754, 0.989, 0.964, 0.642, 0.743, 0.847, 0.882, 0.645, 0.789, 0.905, 0.8, 0.614, 0.982, 0.695, 0.854, 0.754, 0.942, 0.898, 0.947, 0.836, 0.924, 0.906, 0.877, 0.761, 0.889, 0.895, 0.776, 0.438, 0.986, 0.766, 0.241, 0.486, 0.903, 0.706, 0.81, 0.691, 0.542, 0.393, 0.894, 0.959, 0.934, 1.0, 0.836, 0.908, 0.917, 0.78, 0.988, 0.765, 0.877, 0.871, 0.907, 1.0, 0.982, 1.0, 0.827, 1.0, 0.943, 0.946, 0.952, 0.984, 0.983, 1.0, 0.657, 0.466, 0.81, 0.824, 0.938, 0.951, 0.815, 0.865, 0.89, 0.586]
        # a = np.array(a)
        # b = np.array(b)
        # a = b - a
        # b = a/b
        # print(b.tolist())
        # c = [0.759, 0.157, 1.0, 0.851, 0.042, 1.154, 0.924, 0.344, 0.389, 0.58, 0.6, 0.639, 0.661, 0.094, 0.114, 0.716, 0.188, 0.469, 0.123, 0.193, 1.018, 0.792, 0.714, 0.458, 0.676, 0.387, 0.521, 0.176, 0.169, 0.349, 0.764, 0.585, 1.183, 0.87, 0.681, 0.847, 0.253, 0.41, 0.348, 0.297, 0.477, 0.164, 0.653, 0.329, 0.119, 0.125, 0.219, 0.047, 0.086, 0.028, 0.371, 0.588, 0.114, 0.176, 0.181, 0.279, 0.273, 0.055, 0.23, 0.19, 1.18, 0.969, 0.6, 0.475, 0.506, 0.235, 0.246, 0.4, 0.241, 0.164, 0.364, 0.028, 0.72, 0.3, 0.014, 0.068, 0.161, 0.063, 0.0, 0.206, 1.299, 1.259, 0.873, 0.216, 0.484, 0.41, 0.508, 0.568, 0.507, 0.171]
        # print(c.tolist())
        # assert 0
        ...
        self.best_result = {"success_rate":0.0, "average_reward": 0.0, "average_turn": 0,"average_wrong_disease":10,"average_match_rate2":0.0,'average_inquire_human':2.0,'average_activate_human':3.0,'average_ban':1.0}
    ...
    def simulate(self, epoch_number, train_mode=False):
        ...
        for index in range(0, epoch_number,1):
            ...
            #print(index)
            if index > 3000 and index % 3 ==1 :
                print("activator_human_prob",[float(format(np.mean(i), '.3f')) for i in self.rec_human_arr.values()])
                # self.dialogue_manager.disease_replay ={'train': deque(maxlen=6000), 'test': deque(maxlen=6000)}
                # result = self.evaluate_model(dataset="test", index=index)#validate
                # for i in range(2000):
                #     self.evaluate_model(dataset="train", index=index,mode = 'train',evaluate_session_number = 1000)
                #     self.dialogue_manager.train_deep_learning_classifier(epochs=50)
                result = self.evaluate_model(dataset="test", index=index)#validate
                
                
                # a.append(result["success_rate"])
                # b.append(result["average_activate_human"])
                # print('acc',np.mean(a))
                # print('aah',np.mean(b))
                # if len(a) >10 - 1:
                #     a = []
                ...
                if result["average_activate_human"] < self.best_result["average_activate_human"] and \
                    ...
                    result["success_rate"] >= 0.828 and\
                        train_mode==True:
                        #result["average_wrong_disease"] <= self.best_result["average_wrong_disease"] and \
                        #result["average_match_rate2"] < self.best_result["average_match_rate2"] and \
                    #result["average_activate_human"]  < 1  and result["average_turn"]  < 13 and\
    ...
    def evaluate_model(self, dataset, index, mode='test',evaluate_session_number= 6000):
        """
        ...
        evaluate_session_number = evaluate_session_number
        ...
        # self.dialogue_manager.state_tracker.user.set_max_turn(max_turn=20-index)
        self.parameter['max_turn'] = 20 - index
        self.dialogue_manager.theta = 0.98 #- index *0.05
        # print(self.dialogue_manager.theta)
        if self.parameter['max_turn'] < 2:
            assert 0
        for goal_index in evaluate_session_index:
            ...
            num_of_inquire_human_for_group = 0
            num_of_sx_true_for_group = 0
            num_of_sx_all_human_for_group = 0
            while episode_over == False:
                ...
                reward, episode_over, dialogue_status,slots_proportion_list,inquire_state,return_disease = self.dialogue_manager.next(
                    save_record=False,greedy_strategy=False, index=index,mode=mode,SC = self.SC)
                ...
                if episode_over == False:
                    if self.dialogue_manager.state_tracker.agent.lower_agent_is_human is True:
                        ...
                        num_of_inquire_human_for_group += 1
                ...
                elif self.dialogue_manager.state_tracker.agent.ban_human is True:
                    num_of_activate_human += 1
                        
                        
                    num_of_activate_human_for_group +=1
                # if self.dialogue_manager.state_tracker.turn == 43:
                #     self.record_mid_process(self.dialogue_manager.state_tracker.turn, self.dialogue_manager.state_tracker.agent.lower_agent_is_human, subtask_state, inquire_state, return_disease)
                total_reward += reward
            ...
            num_of_sx_true_for_group = slots_proportion_list[0]
            num_of_ban  += slots_proportion_list[1]
            real_implicit_slots += slots_proportion_list[2]
            num_of_sx_all_human_for_group = slots_proportion_list[2]
            total_turns += (self.dialogue_manager.state_tracker.turn-3)/2
            inform_wrong_disease_count += self.dialogue_manager.inform_wrong_disease_count
            self.record_each_group(self.dialogue_manager.state_tracker.user.goal['disease_id'], dialogue_status, num_of_activate_human_for_group,num_of_sx_true_for_group,num_of_sx_all_human_for_group,num_of_inquire_human_for_group)
            if dialogue_status == dialogue_configuration.DIALOGUE_STATUS_SUCCESS:
                success_count += 1
                if self.dialogue_manager.inform_wrong_disease_count == 0:
                    absolute_success_count += 1
        ...
        #ave_activate_human
        # print([self.each_group[iter]['ave_inquire_human'] for iter in range(90) if iter in self.each_group.keys()])
        ...
        # print(data_pro)
        # print([i['recall_sx'] for i in self.each_group.values()])
        # assert 0
        # self.record_early_stop[self.SC] = res
        # with open('estop/'+str(self.SC)+"estop_data.pkl", "wb") as f:
        #     pickle.dump(self.record_early_stop, f)
        # print(self.record_early_stop)
        # self.SC -= 0.01
        if index % 10 ==9:
        ...
        return res
    def record_each_group(self, group_id, dialogue_status,num_of_activate_human,num_of_sx_true_for_group,num_of_sx_all_human_for_group,num_of_inquire_human_for_group):
        ...
        if "all_number" not in self.each_group[group_id].keys():
            ...
            self.each_group[group_id]["inquire_human_number"] = 0
            self.each_group[group_id]["true_sx"] = 0
            self.each_group[group_id]["all_sx"] = 0
            self.each_group[group_id]["recall_sx"] = 0
        self.each_group[group_id]["all_number"] += 1
        self.each_group[group_id]["activate_human_number"] += num_of_activate_human
        self.each_group[group_id]["inquire_human_number"] += num_of_inquire_human_for_group
        self.each_group[group_id]["true_sx"] += num_of_sx_true_for_group
        self.each_group[group_id]["all_sx"] += num_of_sx_all_human_for_group
        ...
        self.each_group[group_id]["ave_inquire_human"] = float("%.3f" % (self.each_group[group_id]["inquire_human_number"] / self.each_group[group_id]["all_number"]))
        if dialogue_status == dialogue_configuration.DIALOGUE_STATUS_SUCCESS:
        ...
        if self.each_group[group_id]["all_sx"] != 0:
            self.each_group[group_id]["recall_sx"] = float("%.3f" % (self.each_group[group_id]["true_sx"] / self.each_group[group_id]["all_sx"]))
