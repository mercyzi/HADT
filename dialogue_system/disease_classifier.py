...
class Net(torch.nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        #两个卷积层
        self.conv1 = torch.nn.Conv2d(3, 3, kernel_size=3)  #3为in_channels 3为out_channels
        self.conv2 = torch.nn.Conv2d(3, 3, kernel_size=3)
        #池化层
        self.pooling = torch.nn.MaxPool2d(2)  #2为分组大小2*2
        #全连接层 
        self.fc1 = torch.nn.Linear(8748, 480)
        self.fc2 = torch.nn.Linear(480, 92)
        self.fc3 = torch.nn.Linear(92, output_size)
...
class dl_classifier(object):
    ...
    def train_dl_classifier(self, epochs):
        ...
        # self.restore_model("/home/zxh/HRL_3/src/dialogue_system/run/disease_classifier0.702836.pkl")
        self.cv = CountVectorizer(tokenizer=tokenizer)
        features = []
        goal_set = pickle.load(open(parameter.get("goal_set"), 'rb'))
    ...
    def test_dl_classifier(self):
        ...
        test_acc = num_correct / len(self.test_batch)
        if test_acc> self.best_acc:
            self.best_acc = test_acc
            print("update__",test_acc)
            # self.best_model.load_state_dict(self.model.state_dict())
            if test_acc> 0.812:
                torch.save(self.model.state_dict(), 'disease_classifier'+format(test_acc,'5f')+'.pkl')
        
        # print("the test accuracy is ", test_acc,"best_acc =", self.best_acc)
    ...
    def create_data(self, train_mode):
        ...
        if train_mode==True:
            ...
            # re_train_set2 = copy.deepcopy(total_set)
            ...
            # total_set = enrich_data(total_set)
        else:
        ...
        for i, dialogue in enumerate(total_set):
            ...
            # print(symp)
            # print(self.slot_set)
            slots_exp = self.cv.transform([self.make_ml_features(symp)]).toarray().squeeze()
...
if __name__ == "__main__":
    ...
    file0='/home/zxh/HRL_sy3/src/data/synthetic_dataset'
    ...
    parser.add_argument("--batch_size", dest="batch_size", type=int, default=1000, help="the batch size when training.")
    ...
    #     state_solts = copy.deepcopy(goal['goal']['explicit_inform_slots'])
    #     state_solts.update(goal['goal']['implicit_inform_slots'])
    #     feature = make_ml_features(state_solts)
    #     features.append(feature)
    epochs = 5000000
    # _x_train = cv.fit_transform(features)
    
    model = dl_classifier(input_size=312, hidden_size=256,
                                   output_size=90,
                                   parameter=parameter)
