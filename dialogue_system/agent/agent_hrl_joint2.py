...
class AgentHRL_joint2(object):
    def __init__(self, action_set, slot_set, disease_symptom, parameter):
        ...
        if self.parameter.get("data_type")=='simulated':
            ...
            self.input_size_dqn_all = {1:1,2:1,3:1,4:1,5:1}
        else:
            raise ValueError
        ...
        self.reward_record = []
        temp_parameter = {}
        self.all_master_time = []
        self.all_machine_time = []
        # Worker human
        ...
        self.activator_m = DQN2(input_size=input_size,
        ...
        if parameter.get("machine_trained") is True and parameter.get("train_mode") is True:
            ...
            self.activator_m.restore_model('/home/zxh/HRL_sy3/src/model/DQN/checkpoint/0609154310_agenthrljoint2_DA0.7_RFH-2_ST3_T20_ss100_lr0.0005_wfrs5.0_RFS30_RFF0_RFNCY0.0_RFIRS0_RFRA-44_RFRMT-44_mls0_gamma1_gammaW0.9_epsilon0.1_awd0_crs0_hwg0_wc0_var0_sdai0_dtft0_ird0_ubc0.985_lbc1e-10_dataSynthetic_Dataset_RID0/lower/activator_m/s0.826_r24.77_t14.008_aah0.454_mr2-0.986_e-3048.pkl')
            self.activator_m.current_net.eval()
            self.activator_m.target_net.eval()
            #
    ...
    def next(self, state, turn, greedy_strategy, **kwargs):
        ...
        asked_h_m = kwargs.get("asked_h_m")
        # print(asked_h_m)

        # print(state["current_slots"]['inform_slots'])
        values = copy.deepcopy(asked_h_m)
        new_state = copy.deepcopy(state)
        features = copy.deepcopy(state["current_slots"]["inform_slots"])
        # 从右向左遍历 values
        for i, val in enumerate(reversed(values)):
            if val == 0:
                # 计算对应的字典 key
                key_to_remove = list(features.keys())[-(i+1)]
                features.pop(key_to_remove)
        # print(features)
        new_state["current_slots"]["inform_slots"] = features
        # print(disease_tag)
        # assert 0
        ...
        if self.parameter.get("state_reduced"):
            ...
            new_state_rep = reduced_state_to_representation_last(state=new_state,
                                                             slot_set=self.slot_set,
                                                             parameter=self.parameter)  # sequence representation.
        else:
        ...
        if self.subtask_terminal == True:
            ...
            end3 = time.time()
            self.all_master_time.append(end3-end2)
        ...
        if self.master_action_index > 1:  # The disease classifier is activated.
            ...
            if self.master_action_index == 0:
                ...
                self.lower_agent_is_human = False
                self.subtask_turn += 1
                agent_action, lower_action_index = self.id2lowerAgent[str(self.current_lower_agent_id)].next(state, self.subtask_turn, greedy_strategy=greedy_strategy,disease_tag = disease_tag,new_state=state)
                asked_h_m.append(1)
            else:
        ...
        end5 = time.time()
        self.all_machine_time.append(end5-end4)
        # print('程序运行时间为: %s Seconds'%(end-end5))

        # print('master程序运行时间为: %s Seconds'%(sum(self.all_master_time[1:]) / len(self.all_master_time)))
        # print('jiqi程序运行时间为: %s Seconds'%(sum(self.all_machine_time[1:]) / len(self.all_machine_time)))


        return agent_action, self.master_action_index, activator_action_index, lower_action_index, asked_h_m
    def parallel_next(self, goal):
        ...
        self.__activator_m_next(state_rep=state_rep, greedy_strategy=False)
        self.current_lower_agent_id = self.master_action_space[self.activator_m_action_index]
        while turn < self.subtask_max_turn and subtask :
            agent_action, lower_action_index = self.id2lowerAgent[str(self.current_lower_agent_id)].next(expect_state, turn, greedy_strategy=False)
    ...
    def __master_next(self, state_rep, greedy_strategy, turn):
        ...
        # if self.master_action_index == 1 :
        #     self.master_action_index = 0
        # self.master_action_index = 1
        if self.ban_human is True and (self.master_action_index == 1 or 2):
            self.master_action_index = 3
    ...
    def record_training_sample(self, state, agent_action, reward, next_state, episode_over, lower_reward, master_action_index, activator_action_index,imp_recall,state_turn,reward_for_activate_doctor_amend):
        ...
        if self.subtask_terminal or episode_over is True:
            ...
            if episode_over is True:# 达到最大轮次或激活了疾病分类器
                ...
                # else:
                if imp_recall >= 0.99 :
                    self.real_master_reward += self.parameter.get("reward_for_success") * 0.5
