...
class DialogueManager_HRL(object):
    ...
    def next(self, greedy_strategy, save_record, index,mode='train',SC = 0.98):
        """
        ...
        def delete_item_from_dict(item, value):
            ...
            return new_item
        def calibrate(probs):
            calibrated_probs = probs ** (1. / 0.99)  # Temper
        ...
        agent_action, master_action_index, activator_action_index, lower_action_index, self.asked_h_m = self.state_tracker.agent.next(state=state, turn=self.state_tracker.turn,
                                                                                              greedy_strategy=greedy_strategy, index=index,
                                                                                              disease_tag = self.state_tracker.user.goal, asked_h_m= self.asked_h_m)
        ...
        softmax_all_Y = F.softmax(all_Y)
        cal_all_Y = calibrate(softmax_all_Y)
        disease_conf = cal_all_Y.max(-1)[0].item()

        early_stop = False
        if disease_conf > self.theta:
            self.state_tracker.agent.ban_human = False
            early_stop = True
        ...
        def slice_dict_by_pattern(flags, values):
            # 找到满足条件的位置：flag[i] == 1 且 flag[i-1] == 0
            start_idx = None
            ...
            # 如果找到，截取 values 从该位置到最后
            if start_idx is not None:
                return start_idx, dict(list(values.items())[start_idx:])
            else:
                return start_idx, {}
        if episode_over == True:
            ...
            real_imp_slots = dict(list(current_slots.items())[real_explicit_slots:])
            start_idx, real_result = slice_dict_by_pattern(self.asked_h_m, real_imp_slots)
            for idx_v, values in enumerate(real_imp_slots.values()):
                if values != dialogue_configuration.I_DO_NOT_KNOW and self.asked_h_m[idx_v] == 1:
                    imp_acc += 1
            # print(self.asked_h_m)
            if len(real_result) >= 1:
                real_asked_h_m = self.asked_h_m[start_idx:]
                for idx_v, values in enumerate(real_result.values()):
                    if real_asked_h_m[idx_v] == 1:
                        all_imp_nums += 1
                        if values != dialogue_configuration.I_DO_NOT_KNOW:
                            imp_nums += 1
                    
            num_of_all_slots = len(current_slots)
            slots_proportion_list.append(imp_nums)  #only imp patient have
    ...
    def build_deep_learning_classifier(self):
        ...
        # for goal in self.state_tracker.user.goal_set['test']:
        #     state_solts = copy.deepcopy(goal['goal']['explicit_inform_slots'])
        #     state_solts.update(goal['goal']['implicit_inform_slots'])
        #     feature = self.make_ml_features(state_solts)
        #     features.append(feature)
        _x_train = self.cv.fit_transform(features)
        self.model = dl_classifier(input_size=_x_train.toarray().shape[1] , hidden_size=256,
                                    output_size=len(self.disease_symptom),
                                    parameter=self.parameter)
        ...
        #                             output_size=9,
        #                             parameter=self.parameter)
        saved_model = '/home/zxh/HRL_sy3/src/dialogue_system/disease_classifier0.829500.pkl'
